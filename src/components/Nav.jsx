...
const Nav = ({cart}) => {
  ...
  return (
    ...
    <div className='flex justify-between mt-2 p-5 items-center list-none md:max-w-[1440px] gap-4 mx-auto border-b border-b-[#F2F0F1]'>
        ...
        <div onClick={handleOpen} className='cursor-pointer md:flex hidden gap-7'>
            ...
            <div className={open ? 'hidden ' : 'absolute z-11 p-8 rounded-xl bg-white shadow-[2px_2px_17px_-2px_rgba(0,_0,_0,_0.1)] mt-8'}>
                <ul className=' flex flex-col text-left gap-3 '>
                    <li onClick={() => setToggle('all')} className={`cursor-pointer hidden md:flex ${toggle === 'all' ? 'border-b w-5 border-b-black' : ''}`}><Link to={"/shop"}>All</Link> </li>
                    <li>T-shirts</li>
                    <li>Shorts</li>
                    <li>Shirts</li>
                    <li>Hoodie</li>
        ...
        {/* Updated Nav Links */}
        <li onClick={() => setToggle('about')} className={`cursor-pointer hidden md:flex ${toggle === 'about' ? 'border-b border-b-black' : ''}`}>
            <Link to={'/about'}>About Us</Link>
        </li>
        <li onClick={() => setToggle('blog')} className={`cursor-pointer hidden md:flex ${toggle === 'blog' ? 'border-b border-b-black' : ''}`}>
            <Link to={'/blog'}>Blog</Link>
        </li>
        <li onClick={() => setToggle('contact')} className={`cursor-pointer hidden md:flex ${toggle === 'contact' ? 'border-b border-b-black' : ''}`}>
